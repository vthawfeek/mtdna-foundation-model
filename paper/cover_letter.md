...
I am submitting a new preprint entitled **"mtDNA-FM: A Domain-Specialized Foundation Model for Mitochondrial DNA with Circular Positional Encoding and Heteroplasmy Modeling"** for posting in the Bioinformatics subject area.

This manuscript presents the first foundation model dedicated to mitochondrial DNA (mtDNA). Existing genomic foundation models (DNABERT-2, HyenaDNA, Nucleotide Transformer) are pre-trained on nuclear genomes and use linear positional encodings that misrepresent the circular topology of mtDNA. Our model addresses two specific architectural problems: (1) a circular positional encoding that enforces continuity at the genome junction, removing the artificial discontinuity at the D-loop boundary; and (2) a heteroplasmy projection channel that accepts continuous per-position variant allele fractions, so the model can distinguish heteroplasmic variant states.

All results are zero-shot (no task-specific labels used in training):

- **37.9% zero-shot haplogroup identification** (26-class cosine 5-NN; 3.85% random baseline; 9.9x lift; 95% CI 34.4-41.2%)
...
Code, pre-trained weights, a Gradio demonstration interface, and a DVC reproducibility pipeline are publicly available at github.com/vthawfeek/mtdna-foundation-model and huggingface.co/vthawfeek/mtdna-foundation-model.

This work will interest researchers in genomic foundation models, mitochondrial disease, and population genetics. The circular positional encoding generalizes to any circular genome: bacterial chromosomes, plasmids, viral genomes, and chloroplast DNA.

No conflicts of interest. The manuscript has not been submitted to any journal.
...
5. Upload:
   - Main PDF (compiled from `paper/manuscript/main.tex`)
...
**Recommended for bioRxiv to journal path:**
