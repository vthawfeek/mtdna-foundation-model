...
\mtdnafm{} covers the same positions --- they fall within the first $\sim$11 windows of its
65-window representation (stride~256, window~512) --- yet achieves F1 = 0.055 on H (2 of 40
...
junction. Four biologically and clinically relevant domains are directly in scope.
...
\textit{Extrachromosomal DNA (ecDNA) in cancer.} ecDNA --- large (0.1--10~Mb) circular DNA
elements found in approximately 14\% of human cancers across 17 tumour types
\citep{turner2017extrachromosomal} --- carries amplified oncogenes (MYC, EGFR, CDK4) and
associated super-enhancers on a single circular molecule. Its circular topology enables
multi-way chromatin interactions unavailable on linear chromosomes and underlies the rapid
oncogene amplification that drives tumour evolution and treatment resistance
\citep{kim2020extrachromosomal}. A sequence foundation model for ecDNA with circular PE would
...
cancer cytogenetics. For sequence-level analysis of the ring junction region --- identifying
breakpoint signatures, predicting which gene disruptions drive phenotype --- circular PE
accurately reflects the physical topology that linear PE misrepresents.
...
gap?} The apparent paradox --- that a model with the topologically correct positional encoding
for a circular genome underperforms a model with a structurally incorrect linear encoding ---
is resolved by recognising that the correct inductive bias provides the right foundation, not a
free performance guarantee. Three factors suppress the expected benefit in the current
evaluation.
...
haplogroups, the PE cannot compensate. Second, the D-loop region --- where circular PE is most
consequential, since positions 16,024--576 span the physical junction --- is captured by both
models. The critical failure identified in Observation~2 is in the \emph{representation} of
...
D-loop junction and for distal haplogroups --- that is currently masked by the parameter budget
gap. A controlled ablation (training an otherwise identical model with standard sinusoidal PE,
comparing zero-shot k-NN accuracy) will isolate this contribution in the extended paper.
Until that ablation is available, the correct characterisation is that circular PE is the
principled, biologically motivated architectural choice for a circular genome, and that the
current results provide no evidence \emph{against} this choice: they provide evidence that
scale is the dominant variable at 5.8M parameters.
