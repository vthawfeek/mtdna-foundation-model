...
\mtdnafm demonstrates that a 6M parameter encoder pre-trained exclusively on mitochondrial sequences
encodes evolutionary structure without task-specific supervision: haplogroup identity, archaic
sequence ancestry, and rRNA/protein-coding gene separation are recoverable from the embedding
space. Zero-shot haplogroup accuracy (37.9\%) is 9.9$\times$ the random baseline; DNABERT-2 (117M
parameters, nuclear genomes) achieves 66.3\% on the same task, with the gap attributable to scale
rather than domain alignment (see Limitations).
...
\item \textit{Model scale.} At 6M parameters, \mtdnafm is substantially smaller than general-purpose
genomic LLMs. DNABERT-2 (117M parameters, nuclear-genome pre-training) achieves 66.3\% on the same
zero-shot haplogroup k-NN task despite seeing only 18\% of each genome (Table~\ref{tab:haplogroup}).
The 28.4 percentage-point gap is likely attributable to the 20$\times$ parameter difference rather
than domain specificity; mtDNA-FM's architectural contributions (circular PE, full-genome coverage,
heteroplasmy channel) are independent of parameter count. Scaling to 24M--100M parameters is a
natural next step.
\end{itemize}
