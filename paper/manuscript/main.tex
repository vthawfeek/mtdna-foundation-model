...
\item \textbf{Downstream evaluation} on three mtDNA-specific tasks (haplogroup classification,
variant pathogenicity prediction, heteroplasmy regression) and two zero-shot discovery experiments
(ancient DNA phylogenetic placement, unsupervised gene-type recovery, Section~\ref{sec:results}).
\end{enumerate}
...
Ablation experiments quantifying the contribution of circular PE, the heteroplasmy channel,
and the two-phase curriculum are underway and will appear in the next version of this preprint.
The zero-shot haplogroup result (Section~\ref{sec:haplo}) and correct archaic DNA placement
(Section~\ref{sec:ancient}) provide indirect support: both depend on the embedding quality
produced by the full architecture.
...
($\approx$270 CPU hours). This is a compute limitation, not a model failure: the 50\% zero-shot
...
\caption{\textbf{Haplogroup embedding structure and zero-shot classification.}
...
The heteroplasmy regression head (5-fold cross-validation, Huber loss) is implemented and
evaluated; quantitative results will be reported in the extended paper once the ablation
baseline (model without het channel, $\lambda_\text{het}=0$) is also run for comparison.
The heteroplasmy channel is active in Phase~2 pre-training ($\lambda_\text{het}=0.3$); its
specific contribution to representation quality is part of the planned ablation study.
