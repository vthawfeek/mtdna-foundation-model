...
achieves 37.9\% accuracy (3.85\% random baseline, 9.8$\times$ lift;
...
All evaluation uses a held-out test split from HmtDB sequences stratified by major haplogroup
(80/10/10 train/val/test, \texttt{random\_state=42}). Metrics: accuracy and macro-F1 for
classification; AUROC and AUPR for pathogenicity. Confidence intervals are computed by
1,000-replicate non-parametric bootstrap. Baseline comparisons: 6-mer frequency vectors with
logistic regression, and random performance. Comparison against DNABERT-2 embeddings is
...
(95\% CI 34.4--41.2\%; random baseline 3.85\%, 9.8$\times$ lift;
macro-F1 0.3703; Table~\ref{tab:haplogroup}). The evaluation set comprises 757 sequences
balanced across 26 PhyloTree major haplogroup classes, drawn from 13,884 NCBI human
mitochondrial genomes with haplogroup annotations in their titles; the train library contains
1,509 sequences (60 per class). Figure~\ref{fig:haplogroup} shows the UMAP of 100 sequences
...
(9.8$\times$ above random baseline 3.85\%). No haplogroup labels were used in pre-training.}
...
(9.8$\times$ random baseline of 3.85\%) and AUROC 0.777 on pathogenic variant
