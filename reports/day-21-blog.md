...
> **Correction (2026-05-31):** The performance table in this post contains metrics that were generated by a synthetic smoke-test (`--synthetic` flag in the evaluate CLI), not real model evaluation. Specifically: the haplogroup fine-tuned accuracy (60.8%) and pathogenicity AUROC (0.877) are seeded random draws, not measurements of the trained models. Additionally, the haplogroup fine-tuning model trained on only 247 sequences instead of ~8,921 due to a label-column mismatch bug that silently discarded sub-haplogroup labels like "H1fx" and "J1c5d". Both bugs have been fixed. The corrected haplogroup model has been retrained; real evaluation metrics are in `reports/eval_summary.json`. No real labeled variant evaluation dataset was available, so pathogenicity AUROC remains unknown. The zero-shot results and architectural claims in this post are unaffected.
...
| Task | Random | k-mer freq PCA+LR | mtDNA-FM (zero-shot) | mtDNA-FM (fine-tuned) |
|------|--------|-------------------|---------------------|----------------------|
| Haplogroup (26-class) | ~4% | ~65% | ~50%¹ | **9.2%**² |
| Pathogenicity AUROC | 0.50 | ~0.72 | — | unknown³ |

¹ Zero-shot k-NN from Day 15 — real measurement on phase1_v1 embeddings.
² Real measurement: LoRA r=8, 1,267 sequences, 5 CPU epochs; macro-F1 = 0.65%. Fine-tuning stalled near random loss; model is 2.4× random but did not converge. Previously reported as 60.8% — that was a synthetic artifact (see correction notice above).
³ Previously reported as 0.877 — synthetic artifact; no real labeled variant evaluation dataset was available.
...
<!-- published: https://rokpayprsizors.wordpress.com/2026/05/31/mtdna-fm-is-live-on-huggingface-hub-what-a-research-grade-model-release-actually-requires-3/ -->
